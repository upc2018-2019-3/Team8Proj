...
public class RecruiterAction extends ActionSupport implements SessionAware, RequestAware{
	...
	public String register() {
		...
		City existCity = cityService.getCity(recruiterRegister.getCity());
		if(existCity == null){
			City c= new City();
			c.setCityName(recruiterRegister.getCity());
			cityService.add(c);
		}
		
		String district = recruiterRegister.getDistrict();
		String companyName = recruiterRegister.getCompanyName();
		if(company == null){
			company = new Company();
			company.setCity(cityService.getCity(recruiterRegister.getCity()));
			company.setCompanyName(companyName);
			company.setDistrict(district);
			companyService.insertCompany(company);
		}
		company = companyService.getCompany(recruiterRegister.getCity(), district, companyName);
		recruiter.setCompany(company);
	...
	public void registerJson() throws Exception {
		...
		City existCity = cityService.getCity(city);
		if(existCity == null){
			City c= new City();
			c.setCityName(city);
			cityService.add(c);
		...
		Company company = companyService.getCompany(city, district, companyName);
		if(company == null){
			companyService.insertCompany(company);
		}
		company = companyService.getCompany(city, district, companyName);
